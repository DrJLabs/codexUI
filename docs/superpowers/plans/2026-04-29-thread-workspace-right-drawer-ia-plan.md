...
4. Confirm disabled/deferred rows show their reason as visible drawer text, not only as browser title text.
5. Close the drawer and reopen it by dragging from the right screen edge on mobile.
6. At 768x1024, confirm the drawer remains usable and the composer remains visible.
7. Navigate to the home/new-thread route and confirm right drawer entry points are hidden and right-edge swipe does not open the drawer.
8. Switch to dark theme and repeat steps 1-7.
9. Select a different thread and confirm the header and artifact counts refresh without stale proposal/worktree counts from the previous thread.
