...
### Automations PR review cycle 18 fixes

#### Feature/Change Name
Review-cycle RRULE validation for unsupported monthly and yearly frequencies.
...
3. Run `pnpm run build`.
4. Run `git diff --check`.
5. Attempt to create an automation through the API with `FREQ=MONTHLY`.
6. Create an automation with a supported schedule, then attempt to patch it to `FREQ=YEARLY`.
7. Evaluate legacy/on-disk monthly and yearly RRULEs directly through the scheduler calculator.

#### Expected Results
- API create and patch reject `FREQ=MONTHLY` and `FREQ=YEARLY` with RRULE validation errors.
- Existing scheduler calculator behavior for legacy monthly/yearly records remains unsupported-but-safe and returns no due run.
- Supported minutely, hourly, daily, and weekly RRULEs remain accepted.
