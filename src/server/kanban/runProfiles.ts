import { FULL_ACCESS_KANBAN_RUN_PROFILE_ID, type CodexRunProfile, type KanbanRunProfileApprovalPolicy, type KanbanRunProfileReasoningEffort, type KanbanRunProfileSandboxMode, type KanbanTask } from '../../types/kanban'

export const DEFAULT_KANBAN_RUN_PROFILE_ID = 'workspace-coding'

const BUILTIN_TIMESTAMP = '2026-04-29T00:00:00.000Z'

export const BUILTIN_KANBAN_RUN_PROFILES: CodexRunProfile[] = [
  {
  ...
  {
    id: DEFAULT_KANBAN_RUN_PROFILE_ID,
  ...
  {
    id: FULL_ACCESS_KANBAN_RUN_PROFILE_ID,
...
export type CodexTurnStartRunSettings = {
  model?: string
  effort?: KanbanRunProfileReasoningEffort
...
export function normalizeKanbanRunProfiles(value: unknown): CodexRunProfile[] {
  ...
  for (const profile of BUILTIN_KANBAN_RUN_PROFILES) {
...
export function normalizeKanbanRunProfileId(value: unknown, profiles: CodexRunProfile[]): string {
  ...
  return DEFAULT_KANBAN_RUN_PROFILE_ID
}
...
export function resolveEffectiveKanbanRunProfile(input: {
  ...
  const profileId = input.task.runProfileId || input.defaultRunProfileId || DEFAULT_KANBAN_RUN_PROFILE_ID
  const baseProfile = input.profiles.find((profile) => profile.id === profileId)
    ?? input.profiles.find((profile) => profile.id === DEFAULT_KANBAN_RUN_PROFILE_ID)
    ?? BUILTIN_KANBAN_RUN_PROFILES.find((profile) => profile.id === DEFAULT_KANBAN_RUN_PROFILE_ID)!
  return {
    ...baseProfile,
...
export function resolveCodexTurnStartRunSettings(profile: CodexRunProfile | null | undefined, worktreePath?: string): CodexTurnStartRunSettings {
  const effectiveProfile = profile
    ?? BUILTIN_KANBAN_RUN_PROFILES.find((candidate) => candidate.id === DEFAULT_KANBAN_RUN_PROFILE_ID)!
...
function readSandboxMode(value: unknown): KanbanRunProfileSandboxMode {
  return value === 'read-only' || value === 'danger-full-access' ? value : 'workspace-write'
}

function readApprovalPolicy(value: unknown): KanbanRunProfileApprovalPolicy {
  return value === 'untrusted' || value === 'never' ? value : 'on-request'
}

function readReasoningEffort(value: unknown): KanbanRunProfileReasoningEffort {
