...
const CRON_DEVELOPER_INSTRUCTIONS = `Response MUST end with a remark-directive block.
...
export function buildCronDeveloperInstructions(projectlessInstructions?: string | null): string {
  return [
    CRON_DEVELOPER_INSTRUCTIONS.trim(),
    projectlessInstructions?.trim() || null,
  ].filter(Boolean).join('\n\n')
}
